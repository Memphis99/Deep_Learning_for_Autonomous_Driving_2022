...
### Managed Spot Instance Training (Beta)

Please note that this workflow has not been exhaustively tested so far. Use it
at your own risk. If you discover bugs, please report them on Piazza.

The previous workflow has the disadvantage that if a spot instance is interrupted,
you have to manually resume the training. As more convenient alternative, 
we provide a managed spot instance training. When a spot instance is interrupted, 
the managed training will automatically restart an instnace once a sufficient 
instance capacity is available and resume the training from the last checkpoint.
To launch a training with the managed instance, you can run:

```shell
python aws_managed_start_instance.py
```

In order to change the training arguments or run multiple trainings after each 
other, you can have a look at 
[mtl/scripts/managed_spot_train.py](mtl/scripts/managed_spot_train.py).

Do NOT use managed instances for interactive development.

The managed spot instance workflow is based on persistent spot instance requests.
To stop the persistent spot request for the instance manually, please use
