...
class Model_Task_Distillation(torch.nn.Module):
    def __init__(self, cfg, outputs_desc):
        ...
        self.last_decoder_semseg = DecoderOld(256, list(outputs_desc.values())[0])

        self.last_decoder_depth = DecoderOld(256, list(outputs_desc.values())[1])
    ...
    def forward(self, x):
        ...
        features_sa_semseg = self.self_attention_semseg(features_4x_depth)

        features_sa_depth = self.self_attention_depth(features_4x_semseg)

        features_sum_semseg = torch.add(features_4x_semseg, features_sa_semseg)

        features_sum_depth = torch.add(features_4x_depth, features_sa_depth)

        predictions_4x_final_semseg, _ = self.last_decoder_semseg(features_sum_semseg, features[4])

        predictions_4x_final_depth, _ = self.last_decoder_depth(features_sum_depth, features[4])
