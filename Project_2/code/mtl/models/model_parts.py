...
class Encoder(torch.nn.Module):
    ...
    def _create(self, name, **encoder_kwargs):
        if name not in _basic_block_layers.keys():
            ...
            pretrained = encoder_kwargs.pop('pretrained', True) #changed to true
...
class DecoderDeeplabV3p(torch.nn.Module):
    def __init__(self, bottleneck_ch, skip_4x_ch, num_out_ch):
        ...
        self.conv_one_by_one = ASPPpart(skip_4x_ch, 48, kernel_size=1, stride=1, padding=0, dilation=1)
        self.conv_three_by_three_first = ASPPpart(48 + bottleneck_ch, 256, kernel_size=3, stride=1, padding=1, dilation=1)
        self.conv_three_by_three_second = ASPPpart(256, 256, kernel_size=3, stride=1, padding=1, dilation=1)
    ...
    def forward(self, features_bottleneck, features_skip_4x):
        ...
        features_bottleneck_new = torch.nn.functional.upsample(features_bottleneck, scale_factor=4, mode="bilinear")
        features_skip_4x_new = self.conv_one_by_one(features_skip_4x)

        out = torch.cat((features_skip_4x_new, features_bottleneck_new), 1)
        out = self.conv_three_by_three_first(out)
        out = self.conv_three_by_three_second(out)
...
class DecoderAddSC(torch.nn.Module):
    def __init__(self, bottleneck_ch, skip_4x_ch, num_out_ch):
        super(DecoderAddSC, self).__init__()

        # TODO: Implement a proper decoder with skip connections instead of the following

        self.conv_one_by_one = ASPPpart(skip_4x_ch, 48, kernel_size=1, stride=1, padding=0, dilation=1)
        self.conv_one_by_one_six = ASPPpart(bottleneck_ch, 24, kernel_size=1, stride=1, padding=0, dilation=1)
        self.conv_one_by_one_twelve = ASPPpart(bottleneck_ch, 24, kernel_size=1, stride=1, padding=0, dilation=1)
        self.conv_three_by_three_first = ASPPpart(96 + bottleneck_ch, 256, kernel_size=3, stride=1, padding=1, dilation=1)
        self.conv_three_by_three_second = ASPPpart(256, 256, kernel_size=3, stride=1, padding=1, dilation=1)
        self.features_to_predictions = torch.nn.Conv2d(256, num_out_ch, kernel_size=1, stride=1)
    def forward(self, features_bottleneck, features_skip_4x, features_rate6, features_rate12):
        """
        DeepLabV3+ style decoder
        :param features_bottleneck: bottleneck features of scale > 4
        :param features_skip_4x: features of encoder of scale == 4
        ...
        #       tensors in the same order and of the same shape.
        features_bottleneck_new = torch.nn.functional.upsample(features_bottleneck, scale_factor=4, mode="bilinear")
        features_rate6_new = torch.nn.functional.upsample(features_rate6, scale_factor=4, mode="bilinear")
        features_rate12_new = torch.nn.functional.upsample(features_rate12, scale_factor=4, mode="bilinear")

        features_skip_4x_new = self.conv_one_by_one(features_skip_4x)
        features_rate6_new = self.conv_one_by_one_six(features_rate6_new)
        features_rate12_new = self.conv_one_by_one_twelve(features_rate12_new)

        out = torch.cat((features_skip_4x_new, features_bottleneck_new, features_rate6_new, features_rate12_new), 1)
        out = self.conv_three_by_three_first(out)
        out = self.conv_three_by_three_second(out)
...
class ASPPAddSC(torch.nn.Module):
    def __init__(self, in_channels, out_channels, rates=(3, 6, 9)):
        super().__init__()
        # TODO: Implement ASPP properly instead of the following
        self.conv_one_by_one = ASPPpart(in_channels, 256, kernel_size=1, stride=1, padding=0, dilation=1)
        self.conv_rate_six = ASPPpart(in_channels, 256, kernel_size=3, stride=1, padding=6, dilation=6)
        self.conv_rate_twelve = ASPPpart(in_channels, 256, kernel_size=3, stride=1, padding=12, dilation=12)
        self.conv_rate_eighteen = ASPPpart(in_channels, 256, kernel_size=3, stride=1, padding=18, dilation=18)
        self.conv_global_pooling = ASSPimagelevel(in_channels, 256, kernel_size=1, output_size=1)
        self.conv_one_by_one_final = ASPPpart(256*5, 256, kernel_size=1, stride=1, padding=0, dilation=1)
        self.conv_out = ASPPpart(256, out_channels, kernel_size=1, stride=1, padding=0, dilation=1)

    def forward(self, x):
        # TODO: Implement ASPP properly instead of the following
        one = self.conv_one_by_one(x)
        two = self.conv_rate_six(x)
        three = self.conv_rate_twelve(x)
        four = self.conv_rate_eighteen(x)
        five = self.conv_global_pooling(x)
        five = torch.nn.functional.upsample(five, size=x.size()[2:], mode="bilinear")
        out = torch.cat((one, two, three, four, five), 1)
        out = self.conv_one_by_one_final(out)
        out = self.conv_out(out)

        return out, two, three
...
class ASSPimagelevel(torch.nn.Sequential):
